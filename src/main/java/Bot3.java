public class Bot3 extends Player implements Decisions{
	...
	public double cardValue(Card card){
		...
		for(int c = 0; c < deck.getDiscardPile().size(); c++) //loop through each individual card in the discard pile
			if(deck.getDiscardPile().get(c) == card) //if the current card in the discard equals to that card
		...
		for(int t = 0; t < markets.length; t++) //loop through my trading areas
			if(!markets[t].isEmpty() && markets[t].getCard() == card) //if the current trading area is not empty and it contains that card
